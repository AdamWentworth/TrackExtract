...
def run_worker(command: list[str], result_path: Path, log_path: Path, provider_name: str, job_id: str) -> tuple[list[dict], Path]:
    ...
    return [
        stem_file(item["label"], Path(item["path"]), job_id)
        for item in result.get("stems", [])
    ], log_path
